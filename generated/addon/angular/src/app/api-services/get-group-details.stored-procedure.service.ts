...
import 'rxjs/add/operator/map';
import { GetGroupDetails } from '../api-models/get-group-details.input-model'
import { GetGroupDetailsViewModel } from '../api-models/get-group-details.view-model'
...
export class GetGroupDetailsService {
    ...
    private getGroupDetailsUrl: string = `${environment.apiUrl}/getgroupdetail`;
    ...
    getGroupDetails(getGroupDetails : GetGroupDetails): Promise<GetGroupDetailsViewModel[]> {
        let body = JSON.stringify(getGroupDetails);
        return this.httpClient.post(this.getGroupDetailsUrl, body, { headers: this.contentHeaders })
            .toPromise()
