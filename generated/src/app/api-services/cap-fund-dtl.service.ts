...
export class CapFundDtlService {
    ...
    updateCapFundDtl(capFundDtl : CapFundDtl, capFundModelId : string): Observable<any> {
        let body = JSON.stringify(capFundDtl);
        return this.httpClient.put(`${this.capFundDtlUrl}/${capFundModelId}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError(this.sharedService.handleError))
    }
