...
export class UserDefinedAttributesService {
    ...
    updateUserDefinedAttributes(userDefinedAttributes : UserDefinedAttributes, winId : string): Observable<any> {
        let body = JSON.stringify(userDefinedAttributes);
        return this.httpClient.put(`${this.userDefinedAttributesUrl}/${winId}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError(this.sharedService.handleError))
    }
