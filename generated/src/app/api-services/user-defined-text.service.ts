...
export class UserDefinedTextService {
    ...
    updateUserDefinedText(userDefinedText : UserDefinedText, winId : string): Observable<any> {
        let body = JSON.stringify(userDefinedText);
        return this.httpClient.put(`${this.userDefinedTextUrl}/${winId}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError(this.sharedService.handleError))
    }
