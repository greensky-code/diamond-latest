...
export class NoteWinLinkService {
    ...
    updateNoteWinLink(noteWinLink : NoteWinLink, winId : string): Observable<any> {
        let body = JSON.stringify(noteWinLink);
        return this.httpClient.put(`${this.noteWinLinkUrl}/${winId}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError(this.sharedService.handleError))
    }
