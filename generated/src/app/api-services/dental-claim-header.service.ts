...
export class DentalClaimHeaderService {
    ...
    private dentalClaimHeaderUrl: string = `${environment.apiUrl}/dentalclaimheaders`;
    ...
    getDentalClaimHeaders(usePagination: boolean=false, page: number = 0, size: number = 0): Observable<DentalClaimHeader[]> {
        var url = `${this.dentalClaimHeaderUrl}/?use-pagination=${usePagination}&page=${page}&size=${size}`;
    ...
    getDentalClaimHeadersCount(): Observable<number> {
        var url = `${this.dentalClaimHeaderUrl}/count`;
        return this.httpClient.get(url, {observe: 'response'})
