...
export class PmbArCustomerMasterService {
    ...
    getPmbArCustomerMaster(customerType : string): Observable<PmbArCustomerMaster> {
        return this.httpClient.get(`${this.pmbArCustomerMasterUrl}/${customerType}`, {observe: 'response'})
            .pipe(map(response => response.body as PmbArCustomerMaster),
                catchError(this.sharedService.handleError))
    }
    ...
    updatePmbArCustomerMaster(pmbArCustomerMaster : PmbArCustomerMaster, customerType : string): Observable<any> {
        let body = JSON.stringify(pmbArCustomerMaster);
        return this.httpClient.put(`${this.pmbArCustomerMasterUrl}/${customerType}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError(this.sharedService.handleError))
    }
