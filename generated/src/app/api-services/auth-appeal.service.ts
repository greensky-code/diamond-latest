...
export class AuthAppealService {
    ...
    private authAppealUrl: string = `${environment.apiUrl}/authappeals`;
    ...
    getAuthAppeals(usePagination: boolean=false, page: number = 0, size: number = 0): Observable<AuthAppeal[]> {
        var url = `${this.authAppealUrl}/?use-pagination=${usePagination}&page=${page}&size=${size}`;
    ...
    getAuthAppeal(authNumber : number): Observable<AuthAppeal> {
        return this.httpClient.get(`${this.authAppealUrl}/${authNumber}`, {observe: 'response'})
            .pipe(map(response => response.body as AuthAppeal),
                catchError(this.sharedService.handleError))
    }

    getAuthAppealsCount(): Observable<number> {
        var url = `${this.authAppealUrl}/count`;
        return this.httpClient.get(url, {observe: 'response'})
    ...
    updateAuthAppeal(authAppeal : AuthAppeal, authNumber : number): Observable<any> {
        let body = JSON.stringify(authAppeal);
        return this.httpClient.put(`${this.authAppealUrl}/${authNumber}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError(this.sharedService.handleError))
    }
