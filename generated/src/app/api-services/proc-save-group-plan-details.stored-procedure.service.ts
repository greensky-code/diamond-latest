...
import 'rxjs/add/operator/map';
import { ProcSaveGroupPlanDetails } from '../api-models/proc-save-group-plan-details.input-model'
import { ProcSaveGroupPlanDetailsViewModel } from '../api-models/proc-save-group-plan-details.view-model'
...
@Injectable()
export class ProcSaveGroupPlanDetailsService {

    private procSaveGroupPlanDetailsUrl: string = `${environment.apiUrl}/procsavegroupplandetail`;
    ...
    procSaveGroupPlanDetails(procSaveGroupPlanDetails : ProcSaveGroupPlanDetails): Promise<ProcSaveGroupPlanDetailsViewModel[]> {
        let body = JSON.stringify(procSaveGroupPlanDetails);
        return this.httpClient.post(this.procSaveGroupPlanDetailsUrl, body, { headers: this.contentHeaders })
            .toPromise()
