...
export class MemberAddressService {
    ...
    updateMemberAddress(memberAddress : MemberAddress, seqMembId : number): Observable<any> {
        let body = JSON.stringify(memberAddress);
        return this.httpClient.put(`${this.memberAddressUrl}/${seqMembId}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError(this.sharedService.handleError))
    }
