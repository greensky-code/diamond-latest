...
import 'rxjs/add/operator/map';
import { ProcLoadNote } from '../api-models/proc-load-note.input-model'
import { ProcLoadNoteViewModel } from '../api-models/proc-load-note.view-model'
...
@Injectable()
export class ProcLoadNoteService {

    private procLoadNoteUrl: string = `${environment.apiUrl}/procloadnotes`;
    ...
    procLoadNote(procLoadNote : ProcLoadNote): Promise<ProcLoadNoteViewModel[]> {
        let body = JSON.stringify(procLoadNote);
        return this.httpClient.post(this.procLoadNoteUrl, body, { headers: this.contentHeaders })
            .toPromise()
