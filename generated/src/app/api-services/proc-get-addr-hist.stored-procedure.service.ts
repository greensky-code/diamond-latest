...
import 'rxjs/add/operator/map';
import { ProcGetAddrHist } from '../api-models/proc-get-addr-hist.input-model'
import { ProcGetAddrHistViewModel } from '../api-models/proc-get-addr-hist.view-model'
...
@Injectable()
export class ProcGetAddrHistService {

    private procGetAddrHistUrl: string = `${environment.apiUrl}/procgetaddrhists`;
    ...
    procGetAddrHist(procGetAddrHist : ProcGetAddrHist): Promise<ProcGetAddrHistViewModel[]> {
        let body = JSON.stringify(procGetAddrHist);
        return this.httpClient.post(this.procGetAddrHistUrl, body, { headers: this.contentHeaders })
            .toPromise()
