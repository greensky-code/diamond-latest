...
import {  PriceRuleMaster } from "../../api-models/index"
...
editPriceRuleMaster: boolean;
    ...
    if (this.secWin.hasInsertPermission()) {
        createPriceRuleMaster() {
            ...
            if(this.priceRuleForm.valid) {
                ...
                priceRuleMaster.useExtDrgPricer = Form.getValue(this.priceRuleForm, 'useExternalDrgPricer');
                priceRuleMaster.priceRule = Form.getValue(this.priceRuleForm, 'customPricerRules');
                priceRuleMaster.procedurePriceOption = Form.getValue(this.priceRuleForm, 'procedurePriceSearchOption');
                priceRuleMaster.modifierCode = Form.getValue(this.priceRuleForm, 'wildcardModifier');
                priceRuleMaster.minimumMinForUnit = Form.getValue(this.priceRuleForm, 'minimumMinForAUnit');
                priceRuleMaster.currConvEffDate = Form.getValue(this.priceRuleForm, 'currConvEffecDate');
                this.priceRuleMasterService.createPriceRuleMaster(priceRuleMaster).subscribe(response => {
    ...
    updatePriceRuleMaster(priceRule : string) {
        ...
        if(this.priceRuleForm.valid) {
            ...
            priceRuleMaster.minimumMinForUnit = Form.getValue(this.priceRuleForm, 'minimumMinForAUnit');
            priceRuleMaster.currConvEffDate = Form.getValue(this.priceRuleForm, 'currConvEffecDate');
            this.priceRuleMasterService.updatePriceRuleMaster(priceRuleMaster, priceRule).subscribe(response => {
        ...
        if (!(this.secWin && this.secWin.hasDeletePermission())) {
            ...
            this.priceRuleMasterService.deletePriceRuleMaster(priceRule).subscribe(response => {
        ...
        this.priceRuleMasterService.getPriceRuleMaster(priceRule).subscribe(priceRuleMaster => {
            ...
            this.priceRuleForm.patchValue({
                ...
                'currConvEffecDate': this.dateFormatPipe.defaultDisplayDateFormat(this.priceRuleMaster.currConvEffDate),
            });
        }, error => {
            this.alertMessage = this.alertMessageService.error("An Error occurred while retrieving record.");
        });
    }    getPriceRuleMasters() {
        this.priceRuleMasterService.getPriceRuleMasters().subscribe(priceRuleMasters => {
...
// Use a FormBuilder to create a FormGroup to define the Form Model for the view
    ...
    createForm() {
        ...
        this.priceRuleForm = this.formBuilder.group({
            ...
            procedurePriceSearchOption: ['', {updateOn: 'blur', validators: [] }],
