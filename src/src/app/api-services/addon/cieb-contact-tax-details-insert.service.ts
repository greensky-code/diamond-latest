...
export class CiebContactInsertTaxDetailsService {

    private pricingAccntCodesUrl: string = `${environment.apiUrl}/procinsertvatregno`;
    ...
    getCiebPricingAccntCodes(procGetGroupTaxInput: ProcGetGroupTaxInput, usePagination: boolean = false, page: number = 0, size: number = 0): Observable<CiebContactPlanDetailsModel[]> {
    ...
    updateCiebPricingAccntCode(CiebContactPlanDetailsModel: CiebContactPlanDetailsModel): Observable<any> {
        let body = JSON.stringify(CiebContactPlanDetailsModel);
        return this.httpClient.put(`${this.pricingAccntCodesUrl}`, body, {headers: this.contentHeaders})
            .pipe(map(response => response),
                catchError(this.sharedService.handleError))
    }
