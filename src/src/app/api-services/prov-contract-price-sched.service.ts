...
export class ProvContractPriceSchedService {
    ...
    updateContractPriceSched(provContractPriceSched : ProvContractPriceSched[]): Observable<any> {
        let body = JSON.stringify(provContractPriceSched);
        return this.httpClient.post(`${this.provContractPriceSchedUrl}/updateContractPriceSched`, body, { headers: this.contentHeaders })
            .pipe(map(response => response));
    }
}
