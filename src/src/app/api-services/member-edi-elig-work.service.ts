...
export class MemberEdiEligWorkService {
    ...
    getMemberEdiEligWork(subscriberId:string, personNumber : string): Observable<MemberEdiEligWork> {
        return this.httpClient.get(`${this.memberEdiEligWorkUrl}/${subscriberId}/${personNumber}`, {observe: 'response'})
            .pipe(map(response => response.body as MemberEdiEligWork),
                catchError(this.sharedService.handleError))
    }
