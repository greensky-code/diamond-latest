...
export class InstClaimHeaderService {
    ...
    findByClaimNumber(value: any, isSilentError: boolean = true): Observable<any> {
        return this.httpClient.get(`${this.instClaimHeaderUrl}/find-by-claimnumber/${value}`, {observe: 'response'})
            .pipe(map(response => response.body as InstClaimHeader),
                catchError((error: any) => {
                    return isSilentError ? error : this.sharedService.handleError(error, null, isSilentError);
                }))
    }

    findByIdAndDate(seqMembId: number, date: string): Observable<InstClaimHeader[]> {
        return this.httpClient.get(`${this.instClaimHeaderUrl}/find-by-idanddate/${seqMembId}/date/${date}`, {observe: 'response'})
            .pipe(map(response => response.body as InstClaimHeader),
                catchError((error: any) => {
                    return this.sharedService.handleError(error)
                }))
    ...
    findById(seqMembId: number): Observable<InstClaimHeader[]> {
        return this.httpClient.get(`${this.instClaimHeaderUrl}/find-by-id/${seqMembId}`, {observe: 'response'})
            .pipe(map(response => response.body as InstClaimHeader),
                catchError((error: any) => {
                    return this.sharedService.handleError(error)
                }))
