...
export class AuthWaiveDeterminantsService {
    ...
    updateAuthWaiveDeterminants(authWaiveDeterminants : AuthWaiveDeterminants, searchSequence: any, seqAwRule : number): Observable<any> {
        let body = JSON.stringify(authWaiveDeterminants);
        return this.httpClient.put(`${this.authWaiveDeterminantsUrl}/${searchSequence}/${seqAwRule}`, body, { headers: this.contentHeaders })
             .pipe(map(response => response),
                 catchError((error: any) => {
                     return this.sharedService.handleError(error)
                 }))
