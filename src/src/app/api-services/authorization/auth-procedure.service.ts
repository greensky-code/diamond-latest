...
import 'rxjs/add/operator/map';
import { AuthProcedure } from '../../../app/api-models/authorization/auth-procedure.model'
...
export class AuthProcedureService {
  ...
  getAuthProcedure(authNumber : number): Observable<AuthProcedure[]> {
      return this.httpClient.get(`${this.authProcedureUrl}/find-by-authNo/${authNumber}`, {observe: 'response'})
          .pipe(map(response => response.body as AuthProcedure[]),
              catchError(this.sharedService.handleError))
  }
  ...
  updateAuthProcedure(authProcedure : AuthProcedure, seqAuthDetail:number, secondaryAuthNo:string, authNumber : number): Observable<any> {
      let body = JSON.stringify(authProcedure);
      return this.httpClient.put(`${this.authProcedureUrl}/${seqAuthDetail}/${secondaryAuthNo}/${authNumber}`, body, { headers: this.contentHeaders })
           .pipe(map(response => response),
               catchError(this.sharedService.handleError))
  }
