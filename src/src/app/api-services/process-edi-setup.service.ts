...
export class ProcessEdiSetupService {
    ...
    getNextJobId(): Observable<string> {
        ...
        return this.httpClient.get(`${this.processEdiSetupUrl}/getNextJobId`, { headers, responseType: 'text'})
    ...
    getNextPrediId(): Observable<string> {
        const headers = new HttpHeaders().set('Content-Type', 'text/plain; charset=utf-8');
        return this.httpClient.get(`${this.processEdiSetupUrl}/getNextPrediId`, { headers, responseType: 'text'})
            .pipe(map(response => response as string)
                ,catchError((error: any) => {
                     return this.sharedService.handleError(error)
                 }));
    ...
    findAllProcessEdiSetups(): Observable<ProcessEdiSetup[]> {
        return this.httpClient.get(`${this.processEdiSetupUrl}/find-all-process-edi-setups`, {observe: 'response'})
            .pipe(map(response => response.body as ProcessEdiSetup),
                catchError(this.sharedService.handleError))
