...
import 'rxjs/add/operator/map';
import { CiebGroupMaster } from '../api-models/addon/cieb-group-master.model'
...
export class CiebGroupMasterService {
    ...
    getCiebGroupMasterByHsdGroupId(hsdGroupId : string): Observable<CiebGroupMaster> {
        return this.httpClient.get(`${this.ciebGroupMasterUrl}/findByHsdGroupId/${hsdGroupId}`, {observe: 'response'})
            .pipe(map(response => response.body as CiebGroupMaster),
                catchError(this.sharedService.handleError))
    }
