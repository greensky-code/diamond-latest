...
export class CustomValidators {
   ...
   public USDate = Validators.pattern(US_DATE_FORMAT);
    ...
    public validateTelephoneFaxNumber(control: any) {
        const tenDigitReg = /^[0-9]{10}$/;
        const sevenDigitReg = /^[0-9]{7}$/;
        const usPhoneNumRegex = US_PHONE_NUMBER_PATTERN;
        const value = control?.value?.trim();
        return (tenDigitReg.test(value) || sevenDigitReg.test(value) || usPhoneNumRegex.test(value)) ?
            null : {'phonefaxnumber': true};
    }
