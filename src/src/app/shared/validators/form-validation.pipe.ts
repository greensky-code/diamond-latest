import {
    ALPHA_CHARACTER_PATTERN,
    EMAIL_ADDRESS_PATTERN,
    FOUR_DIGIT_NUMBER_PATTERN,
    MALE_AND_FEMALE_PATTERN,
    POSITIVE_INTEGER,
    SSN,
    TWO_DIGIT_NUMBER_PATTERN,
    US_DATE_FORMAT,
    US_PHONE_NUMBER_PATTERN,
    YES_NO_AN_UNKNOWN_PATTERN,
    YES_NO_PATTERN,
    ZIPCODE5DIGIT
...
export class FormValidation {
    ...
    errorMessage(field: string) {
        if (!this.form || !this.form.get(field) || !this.form.get(field).errors) {
            return;
        }
        ...
        if (this.form.get(field).errors.min) {
            let requiredLength = this.form.get(field).errors.min.min;
            let actualLength = this.form.get(field).errors.min.actual;
            if (requiredLength == 1) {
                return `This field has a minimum value ${requiredLength}. You entered ${actualLength}.`
            } else if (requiredLength > 1) {
                return `This field has a minimum value ${requiredLength}. You entered ${actualLength}.`
            }
        }
        ...
        if (this.form.get(field).errors.max) {
            let requiredLength = this.form.get(field).errors.max.max;
            let actualLength = this.form.get(field).errors.max.actual;
            if (requiredLength == 1) {
                return `This field has a maximum value ${requiredLength}. You entered ${actualLength}.`
            } else if (requiredLength > 1) {
                return `This field has a maximum value ${requiredLength}. You entered ${actualLength}.`
            }
        }
        ...
        if (this.form.get(field).errors.dateRange) {
            return "This date range field is wrong"
        }
        if (this.form.get(field).errors.passwordMismatch) {
            return 'The Password and the Confirmed Password do not match. Please type them again.';
        }
        if (this.form.get(field).errors.dateRangeMissingToDate) {
            return "This date range field is missing 'to' date"
        }
        if (this.form.get(field).errors.dateRangeMissingFromDate) {
            return "This date range field is missing 'from' date"
        }
        if (this.form.get(field).errors.dateRangeEarlierToDate) {
            return "This date range field has earlier 'to' date"
        }
        ...
        if (this.form.get(field).errors.cannotContainSpace) {
            return `27013: It is required field. Enter something other than blanks.`;
        }
